 ...
 *
 * @cssproperty [--mfe-popover-arrow-display=none] - Sets the display of popovers arrow. Set as `block` to make arrow visible.
 * @cssproperty [--mfe-popover-background-color=--mfe-color-neutral-full] - Sets the background color of popover.
 * @cssproperty [--mfe-popover-border-color=--mfe-color-primary-highlight] - Sets the border color of popover.
 * @cssproperty [--mfe-popover-border-size=1px] - Sets the border size of popover. You can set it to `0px` to not have a border (if you use a custom background color). Always use with a length unit.
 * @cssproperty [--mfe-popover-padding=--mfe-size-m] - Sets the padding of popover.
 * @cssproperty [--mfe-popover-border-radius=--mfe-size-3xs] - Sets the border radius of popover.
 * @cssproperty [--mfe-popover-max-width=100vw] - Sets the maximum width of the popover (including border and padding).
 * @cssproperty [--mfe-popover-position=fixed] - Sets the position of popover. You can set it to `absolute` if parent element is a fixed positioned element like drawer or dialog.
 */
@customElement("mfe-popover")
export default class MfePopover extends LitElement {
  static styles = MfePopoverStyle;
   ...
   */
  @event("mfe-popover-show") private onMfePopoverShow!: EventDispatcher<string>;

  /**
   * Fires when popover becomes hidden
   */
  @event("mfe-popover-hide") private onMfePopoverHide!: EventDispatcher<string>;
          ...
          apply(args: MiddlewareState) {
            if (args.elements.floating && args.elements.reference) {
              Object.assign(args.elements.floating.style, {
                "min-width": `${
                  args.elements.reference.getBoundingClientRect().width
                }px`,
              });
            }
          },
  ...
  private _handleClickOutside = (event: MouseEvent) => {
    ...
    if (
      !eventPath.includes(this._target as HTMLElement) &&
      !eventPath.includes(this)
    ) {
      this.hide();
    }
  };
  ...
  private setPopover() {
    if (this.target) {
      this.popoverAutoUpdateCleanup = autoUpdate(
  ...
  set target(value: string | Element) {
    ...
    if (!target) {
      console.warn(
        "MfePopover target only accepts an Element instance or a string id of a DOM element."
      );
      return;
    }
  ...
  show() {
    this._visible = true;
    this.setPopover();
    this.onMfePopoverShow("");
    document.addEventListener("click", this._handleClickOutside);
    document.addEventListener("keydown", this._handleKeydownEvent);
    document.addEventListener("mfe-popover-show", this._handlePopoverShowEvent);
  }
  ...
  private _handlePopoverShowEvent(event: Event) {
    if (event.target !== this) {
      ...
      if (
        !isNestedPopover &&
        (event.target as HTMLElement).tagName === this.tagName
      ) {
        this.hide();
      }
    }
