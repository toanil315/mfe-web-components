 ...
 *
 * @cssproperty [--mfe-drawer-animation-duration=250ms] Drawer slide in animation duration
 */

@customElement("mfe-drawer")
export default class MfeDrawer extends LitElement {
  static styles = MfeDrawerStyle;
   ...
   */
  @event("mfe-drawer-open") private onOpen!: EventDispatcher<string>;

  /**
   * Fires when the drawer is closed
   */
  @event("mfe-drawer-close") private onClose!: EventDispatcher<string>;
  ...
  connectedCallback() {
    ...
    window?.addEventListener("mfe-drawer-open", (event) => {
    ...
    if (drawerWidth) {
      if (window?.innerWidth < drawerWidth) {
        this.style.setProperty(
          "--mfe-drawer-current-width",
          "calc(100vw - 24px)"
        );
      } else {
        this.style.setProperty("--mfe-drawer-current-width", newWidth);
      }
    }
  }
  ...
  private renderContainer() {
    const title = this.caption
      ? html`<h2 id="drawer-caption">${this.caption}</h2>`
      : "";
