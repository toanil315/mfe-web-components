 ...
 **/
@customElement("mfe-calendar")
export default class MfeCalendar extends DatepickerCalendarMixin {
  @state()
  today = new Date();
  @state()
   ...
   */
  @event(mfeCalendarChangedEvent) _onMfeCalendarChange!: EventDispatcher<
    Date[]
  >;

  static styles = MfeCalendarStyle;
  ...
  get days() {
    return [...Array(7).keys()].map((day) => ({
      name: new Date(0, 0, day).toLocaleString(this.locale, {
        weekday: "short",
      }),
      value: day,
    }));
  ...
  setCurrentCalendarView(view: CalendarView) {
    this._calendarView =
      this._calendarView !== view ? view : CALENDAR_VIEWS.DAYS;
    this.setHoverClass();
  }
  ...
  clearRangePickerStyles() {
    this.shadowRoot
      ...
      .forEach((day) =>
        day.classList.remove("range-day", "range-start-day", "range-end-day")
      );
  }
  ...
  handleDate(date: Date) {
    ...
    this._onMfeCalendarChange(this._selectedDates);
    this.requestUpdate();
  }
  ...
  handleMultipleSelectCalendar(calendarDate: Date) {
    const dateExist = this._selectedDates?.some(
      (d) => d.getTime() === calendarDate.getTime()
    );

    dateExist
      ? this._selectedDates?.splice(
          this._selectedDates?.findIndex(
            (d) => d.getTime() === calendarDate.getTime()
          ),
          1
        )
  ...
  checkIfSelectedDate(calendarDate: Date) {
    return this._selectedDates?.some(
      (date) => date?.getTime() === calendarDate.getTime()
    );
  }
  ...
  createCalendarDays() {
    ...
    const currentMonthStartWeekDay = this.getWeekDayOfDate(
      this._calendarYear,
      this._calendarMonth
    );
    ...
    const currentMonthDayCount = this.getDayNumInAMonth(
      this._calendarYear,
      this._calendarMonth
    );
    ...
    const index = this._calendarDays.findIndex(
      (day) => day.value === dayOfTheWeek % 7
    );
  ...
  renderCalendarHeader() {
    ...
    const showYearSelected =
      this._calendarView === CALENDAR_VIEWS.YEARS ? "header-text-hover" : "";
          ...
          return html` <div class="week-row">
            ${valuesArray.map((values) => {
              ...
              const classes = classMap({
                ...
                "other-month-day":
                  values[key].getMonth() !== this._calendarMonth,
  ...
  renderCalendarMonths() {
    return html` <div class="grid-content">
      ${this.months.map((month, index) => {
        const variant =
          month.value === this._calendarMonth ? "primary" : "tertiary";
        const neutral =
          month.value === this._calendarMonth ? "default" : "neutral";
        ...
        return html` <mfe-button
          ...
          ><span class="calendar-text">${month.name}</span></mfe-button
        >`;
      })}
    </div>`;
  }
    ...
    return html` <div class="grid-content">
      ${this._calendarYears.map((year) => {
        ...
        return html` <mfe-button
          ...
          ><span class="calendar-text">${year}</span></mfe-button
        >`;
      })}
    </div>`;
  }
        ...
        <div class="calendar-content">
          <div class="calendar">
            ...
            ${this._calendarView === CALENDAR_VIEWS.MONTHS
              ...
              : ""}
            ${this._calendarView === CALENDAR_VIEWS.YEARS
